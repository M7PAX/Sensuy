...
<template>
    <div class="w-12 flex flex-col h-full p-2 text-center">
        <Link :href="route('posts.upVote', post.slug)" preserve-scroll method="post" as="button" type="button" class="hover:text-success flex justify-center my-2"
        ...
        <div class="p-2 my-2 font-bold" :class="{
                'text-success': post.voted[0]?.vote === 1,
                'text-error': post.voted[0]?.vote === -1}"
        >
            {{ post.votes }}
        </div>

        <Link :href="route('posts.downVote', post.slug)" preserve-scroll method="post" as="button" type="button" class="hover:text-error flex justify-center my-2"
